...
export const mustGetPane = <Data>(
  ...
  if (foundPane === undefined) {
    throw new Error(
      `no pane found for id: ${paneId}, available panes: ${panes
        .map(({ id }) => id)
        .join(', ')}`,
    );
  }
  ...
  if (foundContent === undefined) {
    throw new Error(
      `no content found in pane: ${
        pane.id
      } for id: ${contentId}, available contents: ${pane.contents
        .map(({ id }) => id)
        .join(', ')}`,
    );
  }
  ...
  return panes
    ...
    .map((pane: IPane<Data>) => {
      ...
      return {
        ...pane,
        childOf:
          pane.childOf === paneUpdate.oldId ? paneUpdate.pane.id : pane.childOf,
